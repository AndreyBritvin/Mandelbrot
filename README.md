...
|-----------|------|------|------|------|------|------|------|
| **Среднее** | 18116 | 7960 | 5357 | 1625 | 24897 | 20896 | 2.5 |
| **Стандартное отклонение** | 33 | 41   | 12   | 143  | 51 | 59 | 0.0050 |
| **Относительная ошибка** | 0.2% | 0.5% | 0.2% | 8.8% | 0.2% | 0.3% | 0.2% |


| Параметр  | SISD (i5 13420h) | SIMD_manual (i5 13420h) | SIMD (i5 13420h) | SIMDT_CPU (i5 13420h) | SISD (i5 12600k) | SIMD_manual (i5 12600k) | SIMT_GPU (i5 12600k + RTX 3070) |
|-----------|------|------|------|------|------|------|------|
| **Относительный прирост** | 1 | 2.27 | 3.38 | 11.15 | 1 | 1.19 | 9622 |
...
Использование CUDA ядер GPU ускорило результат относительно `SISD` в 10000 раз, в то время как при использовании многопоточности на другом процессоре максимальное ускорение составило 11 раз. Хорошо заметно, что использование большого количества ядер для параллельных вычислений хорошо сказывается на производительности. (Спойлер: float в 2.5 - 4 раза быстрее double)
